...
def parse_args(base_parser, args, namespace):
    ...
    parser.add_argument("--eval_seq_prefix", default="none", type=str)
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "d-muon",
            "muon-pytorch",
            "adam-mini",    # needed for OptiSelect paper Table 1
            "galore",       # needed for OptiSelect paper Table 1
            "c-adamw",      # needed for OptiSelect paper Table 1
        ],
    )
    ...
    parser.add_argument("--adema_beta3", default=0.9999, type=float)     # Paper Appendix C: 0.9999
    parser.add_argument("--adema_alpha", default=0.8, type=float)        # Paper Appendix C: 0.8
    parser.add_argument("--adema_beta3_warmup", default=None, type=int)
    parser.add_argument("--adema_alpha_warmup", default=None, type=int)  # Paper: T_α=0 (no warmup)
    ...
    # GaLore-specific (Paper Eq. 21-22)
    parser.add_argument("--galore_rank", default=256, type=int,
                        help="Paper Appendix C: GaLore rank r=256")
    parser.add_argument("--galore_update_proj_gap", default=200, type=int,
                        help="Paper Appendix C: projection update interval")
    parser.add_argument("--galore_scale", default=0.25, type=float)

    # C-AdamW-specific (Paper Eq. 23-24)
    parser.add_argument("--cautious_beta1", default=0.9, type=float)
    parser.add_argument("--cautious_beta2", default=0.999, type=float)

    # Adam-mini-specific
    parser.add_argument("--adam_mini_head_dim", default=64, type=int)
    ...
    parser.add_argument(
        ...
        default=500,
        type=int,
        help="How frequently we save uniform model averages.",
    )
    parser.add_argument(
        "--wa_dtype",
        default="float32",
    ...
    parser.add_argument("--exponential_weight_average", action="store_true")
    parser.add_argument(
        "--ewa_interval",
        default=10,
        type=int,
    ...
    parser.add_argument("--n_kv_head", default=None, type=int)
    ...
    parser.add_argument("--untied_embeds", action="store_true")
    ...
    parser.add_argument("--capacity_factor", default=2.0, type=float)
    parser.add_argument("--moe_num_shared_experts", default=0, type=int)
    parser.add_argument(
        "--moe_router_loss",
    ...
    # MuP arguments
    parser.add_argument("--scale_emb", default=10, type=int)
    parser.add_argument("--scale_base_model", default=256, type=int)
    parser.add_argument("--scale_depth", default=1.4, type=float)

    # ==============================================================
    # OptiSelect: online in-batch data selection
    # Paper: Section 3-4, Appendix C
    # ==============================================================
    parser.add_argument(
        "--selection",
        action="store_true",
        help="Enable OPUS-style online in-batch data selection",
    )
    parser.add_argument(
        "--candidate_multiplier",
        default=2,
        type=int,
        help="Paper Section 3: candidate batch size B̃/B (default: 2)",
    )
    parser.add_argument(
        "--selection_temperature",
        default=0.1,
        type=float,
        help="Paper Appendix C: Boltzmann temperature τ (default: 0.1)",
    )
    parser.add_argument(
        "--selection_sketch_dim",
        default=1024,
        type=int,
        help="Paper Section 4.4: CountSketch dimension m (default: 1024; "
             "exact scoring used at 124M scale, this is recorded but not applied)",
    )
    parser.add_argument(
        "--selection_redundancy_weight",
        default=1.0,
        type=float,
    ...
    parser.add_argument(
        ...
        default=4096,
        type=int,
        help="Paper Appendix C: fixed benchmark proxy set size (default: 4096 documents)",
    )
    parser.add_argument(
        "--val_proxy_refresh",
        default=5000,
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        choices=[None, "sgd", "adamw", "sophia", "lion", "muon"],
        help="Experiment 3 (Paper Section 5.3): override scoring geometry "
             "independent of the optimizer used for updates",
    )
